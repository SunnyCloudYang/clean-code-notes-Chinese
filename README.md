...
- [《代码整洁之道》笔记](#代码整洁之道笔记)
  ...
  - [第三章 函数](#第三章-函数)
    ...
    - [每个函数一个抽象层级](#每个函数一个抽象层级)
      - [自上而下阅读代码：_逐步向下规则_](#自上而下阅读代码逐步向下规则)
    - [Switch 语句](#switch-语句)
    - [使用描述性名称](#使用描述性名称)
    - [函数参数](#函数参数)
      - [常见的单参形式](#常见的单参形式)
  ...
  - [第4章 注释](#第4章-注释)
    ...
    - [好的注释](#好的注释)
      ...
      - [TODO 注释](#todo-注释)
      - [放大](#放大)
      - [公共 API 中的 Javadocs](#公共-api-中的-javadocs)
    - [糟糕的注释](#糟糕的注释)
      - [含糊不清的注释](#含糊不清的注释)
      - [冗余的注释](#冗余的注释)
      - [误导性注释](#误导性注释)
      ...
      - [日志式注释](#日志式注释)
      - [噪声注释](#噪声注释)
      - [当你可以使用函数或变量时，不要使用注释](#当你可以使用函数或变量时不要使用注释)
      - [位置标记](#位置标记)
      - [闭合括号注释](#闭合括号注释)
      - [署名和作者信息](#署名和作者信息)
      - [注释掉的代码](#注释掉的代码)
      ...
      - [不明显的联系](#不明显的联系)
      - [函数头](#函数头)
      - [非公共代码中的 Javadocs](#非公共代码中的-javadocs)
  - [第5章 格式化](#第5章-格式化)
    - [垂直格式化](#垂直格式化)
      - [概念之间的垂直开放性](#概念之间的垂直开放性)
      - [垂直密度](#垂直密度)
      - [垂直距离](#垂直距离)
      - [垂直顺序](#垂直顺序)
    - [水平格式化](#水平格式化)
  ...
  - [总结](#总结)
    - [注释](#注释)
      ...
      - [C2: 过时的注释](#c2-过时的注释)
      - [C3: 冗余的注释](#c3-冗余的注释)
      - [C4: 写得不好的注释](#c4-写得不好的注释)
      - [C5: 注释掉的代码](#c5-注释掉的代码)
    - [环境](#环境)
      - [E1: 构建需要多于一步](#e1-构建需要多于一步)
      - [E2: 测试需要多于一步](#e2-测试需要多于一步)
    - [函数](#函数)
      - [F1: 过多的参数](#f1-过多的参数)
      - [F2: 输出参数](#f2-输出参数)
      - [F3: 标志参数](#f3-标志参数)
      - [F4: 死函数](#f4-死函数)
    - [通用](#通用)
      - [G1: 一个源文件中的多种语言](#g1-一个源文件中的多种语言)
      - [G2: 未实现显而易见的行为](#g2-未实现显而易见的行为)
      - [G3: 边界条件下的错误行为](#g3-边界条件下的错误行为)
      - [G4: 覆盖安全措施](#g4-覆盖安全措施)
      - [G5: 重复](#g5-重复)
      - [G6: 代码处于错误的抽象层次](#g6-代码处于错误的抽象层次)
      - [G7: 基类依赖于它们的派生类](#g7-基类依赖于它们的派生类)
      - [G8: 过多的信息](#g8-过多的信息)
      - [G9: 死代码](#g9-死代码)
      - [G10: 垂直分离](#g10-垂直分离)
      - [G11: 不一致](#g11-不一致)
      ...
      - [G15: 选择器参数](#g15-选择器参数)
      - [G16: 模糊的意图](#g16-模糊的意图)
      - [G17: 错误放置的责任](#g17-错误放置的责任)
      - [G18: 不恰当的静态](#g18-不恰当的静态)
      - [G19: 使用解释性变量](#g19-使用解释性变量)
      - [G20: 函数名称应说明它们的作用](#g20-函数名称应说明它们的作用)
      - [G21: 理解算法](#g21-理解算法)
      - [G22: 使逻辑依赖物理化](#g22-使逻辑依赖物理化)
      - [G23: 优先使用多态而不是 If/Else 或 Switch/Case](#g23-优先使用多态而不是-ifelse-或-switchcase)
      - [G24: 遵循标准约定](#g24-遵循标准约定)
      - [G25: 用命名常量替换魔法数字](#g25-用命名常量替换魔法数字)
      - [G26: 精确](#g26-精确)
      - [G27: 结构优于约定](#g27-结构优于约定)
      - [G28: 封装条件](#g28-封装条件)
      - [G29: 避免否定条件](#g29-避免否定条件)
      - [G31: 隐藏的时间耦合](#g31-隐藏的时间耦合)
      - [G32: 不要随意](#g32-不要随意)
      - [G33: 封装边界条件](#g33-封装边界条件)
      - [G34: 函数应只下降一个抽象层次](#g34-函数应只下降一个抽象层次)
      - [G35: 将可配置数据保持在高层](#g35-将可配置数据保持在高层)
      - [G36: 避免传递导航](#g36-避免传递导航)
    - [命名](#命名)
      - [N1: 选择描述性名称](#n1-选择描述性名称)
      - [N2: 在适当的抽象层次选择名称](#n2-在适当的抽象层次选择名称)
      - [N3: 尽可能使用标准命名法](#n3-尽可能使用标准命名法)
      - [N4: 明确的名称](#n4-明确的名称)
      - [N5: 在长作用域中使用长名称](#n5-在长作用域中使用长名称)
      - [N6: 避免编码](#n6-避免编码)
      - [N7: 名称应描述副作用](#n7-名称应描述副作用)
    - [测试](#测试)
      - [T1: 测试不足](#t1-测试不足)
      - [T2: 使用覆盖率工具！](#t2-使用覆盖率工具)
      - [T3: 不要跳过琐碎的测试](#t3-不要跳过琐碎的测试)
      - [T4: 被忽略的测试是对模糊性的疑问](#t4-被忽略的测试是对模糊性的疑问)
      - [T5: 测试边界条件](#t5-测试边界条件)
      - [T6: 彻底测试靠近错误的地方](#t6-彻底测试靠近错误的地方)
      - [T7: 失败的模式是揭示性的](#t7-失败的模式是揭示性的)
      - [T8: 测试覆盖率模式可以是揭示性的](#t8-测试覆盖率模式可以是揭示性的)
      - [T9: 测试应该快速](#t9-测试应该快速)
